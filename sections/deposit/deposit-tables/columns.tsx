...
export const columns: ColumnDef<AdminRegisterUsers & PaymentDeposits>[] = [
  ...
  },
  {
    id: 'daily',
    header: 'Daily',
    cell: ({ row }) =><CheckboxDaily depositDate={row.original.date} userId = {row.original.user._id} checkboxStatus={row.original.dailyChecked}/>
  },
  {
    id: 'bonus',
    header: 'BONUS',
    cell: ({ row }) => <CheckboxBonus depositDate={row.original.date} userId = {row.original.user._id} checkboxStatus={row.original.bonusChecked}/>
  },
  {
    accessorKey: 'date',
    header: 'TIME',
