...
export const GET = async (request: NextRequest) => {
  ...
  if (!id || !date) {
    return NextResponse.json({ error: 'Authorization token and date are required' }, { status: 401 });
  }
  ...
  try {
    ...
    if (!foundRegister) {
      return NextResponse.json({ error: 'No registration found for the specified date' }, { status: 403 });
    }
